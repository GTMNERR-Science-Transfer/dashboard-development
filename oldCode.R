...
speciesList = "Please select genus before selecting species"
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(width = 3, selectInput("genus", "What genus of Algae do you want data for?", c("All", unique(GeneraData$genus))),
    ...
    column(width = 4, plotOutput("timePlot"),
...
server <- function(input, output) {
  #Update species drop down based on genus input
  output$selectSpecies <- renderUI(selectInput("species", "Select what species you are interested in (optional)", unique(GeneraData$species[GeneraData$genus == input$genus])))
  output$selectDate <- renderUI(selectInput("date", "The following dates have data for your selected algal bloom. Select one to view data on map (optional)", unique(GeneraData$`Sample Date`[GeneraData$genus == input$genus & GeneraData$species == input$species])))
